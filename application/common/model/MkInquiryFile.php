...
// 来稿需求 文件信息 模型
class MkInquiryFile extends Model
{
    // 软删除
    use SoftDelete;
    ...
    public function getList($search_type = '', $field = '', $keyword = '', $limit = 50)
    {
        ...
        if ($search_type != '') {
            ...
            // 字段不为空
            if($search_type == 'or'){//多字段 或 查询

                $query = $this->where(function ($query) use($field_arr, $keyword_arr) {
        ...
        // 返回分页对象
        return $query->where($where)->order('Inquiry_Date desc, id desc')->paginate($limit);
    }
