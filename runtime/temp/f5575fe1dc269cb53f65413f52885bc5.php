<?php /*a:2:{s:76:"D:\largon\laragon\www\ky\application\admin\view\mk_inquiry\form-Inquiry.html";i:1615786782;s:18:"./layout/list.html";i:1615863909;}*/ ?>
<!doctype html>
<html>
<head>
    <meta charset="utf-8">
        ...
        <div class="layui-row">
            ...
            <div class="mainCt mainSec formReset signLimit">
                <div class="mainWrap">
                    ...
                    <form class="layui-form" id="form" method="post" action="<?php echo url('mk_inquiry/save'); ?>">
                        ...
                        <div class="layui-row layui-col-space10">
							...
							<div class="layui-col-xs4">
							    <div class="layui-form-item">
							        ...
							        <div class="layui-input-block">
										<select name="Attention" id="Attention" lay-filter="Attention" lay-search>
											<option value=""><?php echo session('language') == '中文'? '请选择' : 'Please Select'; ?></option>
										</select>
							        </div>
							    </div>
							...
							<div class="layui-col-xs4">
							    <div class="layui-form-item">
							        ...
							        <div class="layui-input-block">
										<select name="Department" id="Department" lay-filter="" lay-search>
											<option value=""><?php echo session('language') == '中文'? '请选择' : 'Please Select'; ?></option>
										</select>
							        </div>
							    </div>
							...
							<div class="layui-col-xs4">
								<div class="layui-form-item">
									...
									<div class="layui-input-block">
										<input name="VAT_Amount" lay-verify="required" autocomplete="off" placeholder="" class="layui-input" type="number">
									</div>
								</div>
							</div>
...
<script>
	...
	layui.use(['form','laydate'], function () {
		...
		form.on('select(Contract_Number)', function (data) {
			...
			$.ajax({
				type: 'get',
				url: "<?php echo url('mk_inquiry/get_info'); ?>",
				data: {'code': code},
				dataType: 'json',
				success: function (res) {
                    ...
                    // 拆分多文件名 追加下拉选项
                    var a_options = '', d_options = '';
                    for (var i = 0; i < data.length; i++) {
                        a_options += '<option value="' + data[i].Attention + '">' + data[i].Attention + '</option>';
                    }
                    for (var i = 0; i < data.length; i++) {
                        d_options += '<option value="' + data[i].Department + '">' + data[i].Department + '</option>';
                    }

                    //select重新追加option
                    $('#Attention').append(a_options);
                    $('#Department').append(d_options);
                    //刷新select选择框渲染
                    form.render("select");
        ...
        // 客户联系人 下拉选择器值发生变化
        form.on('select(Attention)', function (data) {
            var customer = data.value;
            //console.log(code);
            ...
            $.ajax({
                type: 'get',
                url: "<?php echo url('mk_inquiry/get_bm'); ?>",
                data: {'customer': customer},
                dataType: 'json',
                success: function (res) {

                    $('#Department').val(res.bm);
