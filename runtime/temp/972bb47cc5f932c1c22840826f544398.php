<?php /*a:2:{s:67:"D:\largon\laragon\www\ky\application\admin\view\mk_quote\index.html";i:1615786782;s:18:"./layout/list.html";i:1615863909;}*/ ?>
<!doctype html>
<html>
<head>
    <meta charset="utf-8">
				...
				<div class="layui-row">
					<div class="top">
						...
						<!--<div class="global_btn">
							<a href="<?php echo url('mk_quote/create'); ?>" class="layui-btn"><?php echo session('language') == '中文'? '新增' : 'Add'; ?></a>
						</div>-->
					</div>
					<div class="mainCt">
						<div class="mainWrap">
...
<script type="text/html" id="barDemo">
	<div class="table_btn">
		...
		<a class="layui-btn layui-btn-xs file_btn" lay-event="archive"><?php echo session('language') == '中文'? '存档' : 'Archive'; ?></a>
...
<script>
	...
	layui.use('table', function(){
		...
		function commonCols(language,colsData){
			...
			//右侧操作工具
			if (language === '中文') {
				cols.push({title: '操作', toolbar: '#barDemo', fixed: 'right', align: 'center', width: 300});
			}else{
				cols.push({title: 'Action', toolbar: '#barDemo', fixed: 'right', align: 'center', width: 300});
			}
		...
		$('.search_btn').click(function(){
			...
			//表格重载
			/*table.reload('test', {
				url: "<?php echo url('mk_quote/index'); ?>",
		...
		$(function(){
			...
			$(".addCondition").live("click",function(){
				index = parent.layer.open({ //在父窗口打开
					...
					area : ['700px' , '560px'],
					content: "<?php echo url('mk_quote/condition'); ?>",
					end: function () {
		...
		$('#del').click(function () {
			...
			if(cd !== 0){
				...
				layer.confirm('确认删除？', function(index){
					...
					$.ajax({
						type: 'get',
						url: "<?php echo url('mk_quote/batch_delete'); ?>",
						data: {id : idstr},
						// 删除成功
			...
			if(obj.event === 'del'){
				layer.confirm('确认删除？', function(index){
					...
					$.ajax({
						...
						url: replaceEditUrlId("<?php echo url('mk_quote/delete', ['id' => 1]); ?>", id),
						dataType: 'json',
						// 删除成功
						success: function (res) {
							layer.alert(res.msg, {title: '提示'}, function (index) {
				...
				window.location.href =  replaceEditUrlId("<?php echo url('mk_quote/edit', ['id' => 1]); ?>", id);

			}else if(obj.event === 'look'){

				window.location.href =  replaceEditUrlId("<?php echo url('mk_quote/read', ['id' => 1]); ?>", id);
				...
				layer.confirm('确认？', function(index){
					...
					$.ajax({
						type: 'get',
						contentType: 'application/json',
						url: replaceEditUrlId("<?php echo url('mk_quote/archive', ['id' => 1]); ?>", id),
						dataType: 'json',
						// 成功
						success: function (res) {
							layer.alert(res.msg, {title: '提示'}, function (index) {
				...
				// 报价单 打印预览 新开窗口
				window.open(replaceEditUrlId("<?php echo url('mk_quote/print_view', ['id' => 1]); ?>", id));
			}
