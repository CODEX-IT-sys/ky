<?php /*a:2:{s:69:"D:\largon\laragon\www\ky\application\admin\view\mk_invoice\index.html";i:1615786782;s:18:"./layout/list.html";i:1615863909;}*/ ?>
<!doctype html>
<html>
<head>
    <meta charset="utf-8">
				...
				<div class="layui-row">
					<div class="top">
						<div class="position_lead">
                            ...
                            <a class="on"><?php echo session('language') == '中文'? '请款单' : 'Fapiao'; ?></a>
						</div>
						<div class="global_btn">
							<!--<a class="layui-btn com_btn" id="fapiao"><?php echo session('language') == '中文'? '生成开票单' : 'Fapiao'; ?></a>-->
                            <!--<a href="<?php echo url('mk_invoice/create'); ?>" class="layui-btn"><?php echo session('language') == '中文'? '新增' : 'Add'; ?></a>-->
						</div>
					</div>
					<div class="mainCt">
						<div class="mainWrap">
...
<script type="text/javascript" src="/static/js/validator.js"></script>
	...
	<script type="text/html" id="barDemo">
		<div class="table_btn">
			...
			<a class="layui-btn layui-btn-xs printPreview_btn" lay-event="print"><?php echo session('language') == '中文'? '预览打印' : 'Print'; ?></a>
		</div>
	</script>
	...
	<script>
		...
		layui.use('table', function(){
			...
			function commonCols(language,colsData){
                        ...
                        if(colsData[i].Field !== 'id'){
							if (language === '中文') {
								cols.push({
									...
									sort: true, minWidth: 120, totalRow: true
								});
							} else {
								cols.push({
									field: colsData[i].Field,
									title: colsData[i].Field,
									sort: true, minWidth: 180, totalRow: true
								});
							}
                        }
					}
				...
				// 右侧操作工具
				if (language === '中文') {
					cols.push({fixed: 'right', title: '操作', align: 'center', toolbar: '#barDemo', width: 220});
				}else{
					cols.push({fixed: 'right', title: 'Action', align: 'center', toolbar: '#barDemo', width: 220});
				}
			...
			$('.search_btn').click(function(){
				...
				//表格重载
				/*table.reload('test', {
					url: "<?php echo url('mk_invoice/index'); ?>",
			...
			$(".addCondition").live("click",function(){
				index = parent.layer.open({ //在父窗口打开
					...
					area : ['700px' , '560px'],
					content: "<?php echo url('mk_invoice/condition'); ?>",
					end: function () {
				...
				if(obj.event === 'del'){
					layer.confirm('确认删除？', function(index){
						...
						$.ajax({
							...
							url: replaceEditUrlId("<?php echo url('mk_invoice/delete', ['id' => 1]); ?>", id),
							dataType: 'json',
							// 删除成功
					...
					window.location.href =  replaceEditUrlId("<?php echo url('mk_invoice/edit', ['id' => 1]); ?>", id);

				} else if(obj.event === 'look'){

					window.location.href = replaceEditUrlId("<?php echo url('mk_invoice/read', ['id' => 1]); ?>", id);

				} else if(obj.event === 'print'){

					window.open(replaceEditUrlId("<?php echo url('mk_invoice/print_view', ['id' => 1]); ?>", id));
                    ...
                    //生成开票单
                    window.location.href = replaceEditUrlId("<?php echo url('mk_invoice/fapiao', ['id' => 1]); ?>", id);
                }
			});
            ...
            $('#fapiao').click(function () {
                ...
                if(cd !== 0){
                    ...
                    layer.confirm('confirm？', function(index){
                        ...
                        $.ajax({
                            type: 'get',
                            url: "<?php echo url('mk_invoice/fapiao'); ?>",
                            data: {id : idstr},
                            // 成功
                            success: function (res) {
