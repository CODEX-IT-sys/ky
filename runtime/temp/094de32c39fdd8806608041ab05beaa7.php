<?php /*a:2:{s:77:"D:\largon\laragon\www\ky\application\admin\view\pj_client_feedback\index.html";i:1615786782;s:18:"./layout/list.html";i:1615863909;}*/ ?>
<!doctype html>
<html>
<head>
    <meta charset="utf-8">
			...
			<div class="layui-row">
				<div class="top">
					<div class="position_lead">
						...
						<a class="on"><?php echo session('language') == '中文'? '客户反馈' : 'Customer Feedback'; ?></a>
					</div>
					<div class="global_btn">
						<a class="layui-btn com_btn" id="qapl"><?php echo session('language') == '中文'? '质量经理批准' : 'Quality Manager'; ?></a>

						<a href="<?php echo url('pj_client_feedback/create'); ?>" class="layui-btn"><?php echo session('language') == '中文'? '新增' : 'Add'; ?></a>
	...
	$(function(){
		...
		$(".addCondition").live("click",function(){
			index = parent.layer.open({ //在父窗口打开
				...
				area : ['700px' , '560px'],
				content: "<?php echo url('pj_client_feedback/condition'); ?>",
				end: function () {
					...
					// 表格重载
					table.reload('test', {
						url: "<?php echo url('pj_client_feedback/index'); ?>",
	...
	$('#del').click(function () {
		...
		if(cd !== 0){
			...
			layer.confirm('确认删除？', function(index){
				...
				$.ajax({
					type: 'get',
					url: "<?php echo url('pj_client_feedback/batch_delete'); ?>",
					data: {id : idstr},
					// 删除成功
    ...
    $('#qapl').click(function () {
        ...
        if(cd !== 0){
            ...
            layer.confirm('确认操作？', function(index){
                ...
                $.ajax({
                    type: 'get',
                    url: "<?php echo url('pj_client_feedback/batch_qa'); ?>",
                    data: {id : idstr},
                    // 成功
                    success: function (res) {
                        layer.alert(res.msg, {title: '提示'}, function (index) {
		...
		if(obj.event === 'del'){
			  layer.confirm('确认删除？', function(index){
				  ...
				  $.ajax({
					  ...
					  url: replaceEditUrlId("<?php echo url('pj_client_feedback/delete', ['id' => 1]); ?>", id),
					  dataType: 'json',
					  // 删除成功
					  success: function (res) {
						  layer.alert(res.msg, {title: '提示'}, function (index) {
		  	...
		  	window.location.href = replaceEditUrlId("<?php echo url('pj_client_feedback/edit', ['id' => 1]); ?>", id);

	  	} else if(obj.event === 'look'){

		  	window.location.href = replaceEditUrlId("<?php echo url('pj_client_feedback/read', ['id' => 1]); ?>", id);
	  	}
  	});
