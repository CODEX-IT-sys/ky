<?php /*a:2:{s:69:"D:\largon\laragon\www\ky\application\admin\view\mk_inquiry\index.html";i:1615786782;s:18:"./layout/list.html";i:1615863909;}*/ ?>
<!doctype html>
<html>
<head>
    <meta charset="utf-8">
			...
			<div class="layui-row">
				<div class="top">
					<div class="position_lead">
						...
						<a class="on"><?php echo session('language') == '中文'? '来稿需求' : 'Inquiry'; ?></a>
					</div>
					<div class="global_btn">
						<a href="<?php echo url('mk_quote/index'); ?>" class="layui-btn com_btn"><?php echo session('language') == '中文'? '查看报价单' : 'View Quotation'; ?></a>

						<a href="<?php echo url('mk_inquiry/create'); ?>" class="layui-btn"><?php echo session('language') == '中文'? '新增' : 'Add'; ?></a>
...
<script type="text/html" id="barDemo">
	<div class="table_btn">
		...
		<a class="layui-btn layui-btn-xs com_btn" lay-event="add"><?php echo session('language') == '中文'? '文件信息' : 'File Info'; ?></a>
	</div>
</script>
	...
	$('#del').click(function () {
		...
		if(cd !== 0){
			...
			layer.confirm('确认删除？', function(index){
				...
				$.ajax({
					type: 'get',
					url: "<?php echo url('mk_inquiry/batch_delete'); ?>",
					data: {id : idstr},
					// 删除成功
		...
		if(obj.event === 'del'){
			  layer.confirm('确认删除？', function(index){
				  ...
				  $.ajax({
					  ...
					  url: replaceEditUrlId("<?php echo url('mk_inquiry/delete', ['id' => 1]); ?>", id),
					  dataType: 'json',
					  // 删除成功
					  success: function (res) {
						  layer.alert(res.msg, {title: '提示'}, function (index) {
			...
			window.location.href =  replaceEditUrlId("<?php echo url('mk_inquiry/edit', ['id' => 1]); ?>", id);
        	...
        	window.location.href =  replaceEditUrlId("<?php echo url('mk_inquiry/file_index', ['id' => 1]); ?>", id);
    	}
	});
