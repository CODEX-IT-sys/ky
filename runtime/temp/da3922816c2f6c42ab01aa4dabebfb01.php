			...
			<div class="layui-row">
				<div class="top">
					<div class="position_lead">
						...
						<a class="on"><?php echo session('language') == '中文'? '项目描述' : 'Project Profile'; ?></a>
					</div>
...
<script type="text/html" id="barDemo">
	<div class="table_btn">
		<a class="layui-btn layui-btn-xs com_btn" lay-event="view_file"><?php echo session('language') == '中文'? '查看资料' : 'View File'; ?></a>
		...
		<a class="layui-btn layui-btn-xs com_btn" lay-event="data_read"><?php echo session('language') == '中文'? '数据读取' : 'Data Read'; ?></a>
	</div>
</script>
...
layui.use(['table','form'], function(){
	...
	function commonCols(language,colsData){
		...
		if(language === '中文') {
			...
			cols.push({fixed: 'right', title: 'Action', align: 'center', toolbar: '#barDemo', width: 320});
		}
	...
	$(function(){
		...
		$(".addCondition").live("click",function(){
			index = parent.layer.open({ //在父窗口打开
				...
				area : ['700px' , '560px'],
				content: "<?php echo url('pj_project_profile/condition'); ?>",
				end: function () {
					...
					// 表格重载
					table.reload('test', {
						url: "<?php echo url('pj_project_profile/index'); ?>",
		...
		if(obj.event === 'del'){
			  layer.confirm('Confirm？', function(index){
				  ...
				  $.ajax({
					  ...
					  url: replaceEditUrlId("<?php echo url('pj_project_profile/delete', ['id' => 1]); ?>", id),
					  dataType: 'json',
					  // 删除成功
		  	...
		  	window.location.href =  replaceEditUrlId("<?php echo url('pj_project_profile/edit', ['id' => 1]); ?>", id);

	  	} else if(obj.event === 'data_read'){

		  	window.location.href = replaceEditUrlId("<?php echo url('pj_project_profile/data_read', ['id' => 1]); ?>", id);

	  	} else if(obj.event === 'view_file'){

            window.location.href =  replaceEditUrlId("<?php echo url('pj_project_profile/view_file', ['id' => 1]); ?>", obj.data.Project_Name);
        }
  	});
