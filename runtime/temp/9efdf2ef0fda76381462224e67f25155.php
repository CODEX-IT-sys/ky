<?php /*a:2:{s:77:"D:\largon\laragon\www\ky\application\admin\view\pj_contract_review\index.html";i:1615786782;s:18:"./layout/list.html";i:1615863909;}*/ ?>
<!doctype html>
<html>
<head>
    <meta charset="utf-8">
			...
			<div class="layui-row">
				<div class="top">
					<div class="position_lead">
						...
						<a href="#"><?php echo session('language') == '中文'? '项目管理' : 'Project Management'; ?></a>
						<a class="on"><?php echo session('language') == '中文'? '项目汇总' : 'Contract Review'; ?></a>
					</div>
					<div class="global_btn">

						<a class="layui-btn com_btn" id="xmjlpl"><?php echo session('language') == '中文'? '项目经理批准' : 'Project Manager'; ?></a>

						<a class="layui-btn com_btn" id="zjlpl"><?php echo session('language') == '中文'? '总经理批准' : 'General Manager'; ?></a>

						<a href="<?php echo url('create'); ?>" class="layui-btn"><?php echo session('language') == '中文'? '新增' : 'Add'; ?></a>
					</div>
				</div>
				<div class="mainCt">
					<div class="mainWrap">
	...
	$(function(){
		...
		$(".addCondition").live("click",function(){
			index = parent.layer.open({ //在父窗口打开
				...
				area : ['700px' , '560px'],
				content: "<?php echo url('pj_contract_review/condition'); ?>",
				end: function () {
					...
					// 表格重载
					table.reload('test', {
						url: "<?php echo url('pj_contract_review/index'); ?>",
		...
		if(obj.event === 'del'){
			  layer.confirm('确认删除？', function(index){
				  ...
				  $.ajax({
					  ...
					  url: replaceEditUrlId("<?php echo url('pj_contract_review/delete', ['id' => 1]); ?>", id),
					  dataType: 'json',
					  // 删除成功
		  	...
		  	window.location.href =  replaceEditUrlId("<?php echo url('pj_contract_review/edit', ['id' => 1]); ?>", id);

	  	} else if(obj.event === 'look'){

		  	window.location.href = replaceEditUrlId("<?php echo url('pj_contract_review/read', ['id' => 1]); ?>", id);
			...
			layer.confirm('确认？', function(index){
				...
				$.ajax({
					type: 'get',
					contentType: 'application/json',
					url: replaceEditUrlId("<?php echo url('pj_contract_review/sales_admin', ['id' => 1]); ?>", id),
					dataType: 'json',
					// 成功回调
					success: function (res) {
						layer.alert(res.msg, {title: '提示'}, function (index) {
			...
			layer.confirm('确认？', function(index){
				...
				$.ajax({
					type: 'get',
					contentType: 'application/json',
					url: replaceEditUrlId("<?php echo url('pj_contract_review/project_manager', ['id' => 1]); ?>", id),
					dataType: 'json',
					// 成功回调
					success: function (res) {
						layer.alert(res.msg, {title: '提示'}, function (index) {
			...
			layer.confirm('确认？', function(index){
				...
				$.ajax({
					type: 'get',
					contentType: 'application/json',
					url: replaceEditUrlId("<?php echo url('pj_contract_review/general_manager', ['id' => 1]); ?>", id),
					dataType: 'json',
					// 成功回调
					success: function (res) {
						layer.alert(res.msg, {title: '提示'}, function (index) {
	...
	$('#xmjlpl').click(function () {
		...
		if(cd !== 0){
			...
			layer.confirm('确认操作？', function(index){
				...
				$.ajax({
					type: 'get',
					url: "<?php echo url('pj_contract_review/batch_pm'); ?>",
					data: {id : idstr},
					// 成功
					success: function (res) {
						layer.alert(res.msg, {title: '提示'}, function (index) {
	...
	$('#zjlpl').click(function () {
		...
		if(cd !== 0){
			...
			layer.confirm('确认操作？', function(index){
				...
				$.ajax({
					type: 'get',
					url: "<?php echo url('pj_contract_review/batch_gm'); ?>",
					data: {id : idstr},
					// 删除成功
					success: function (res) {
						layer.alert(res.msg, {title: '提示'}, function (index) {
