<?php /*a:2:{s:73:"D:\largon\laragon\www\ky\application\admin\view\mk_feseability\index.html";i:1615786782;s:18:"./layout/list.html";i:1615863909;}*/ ?>
<!doctype html>
<html>
<head>
    <meta charset="utf-8">
			...
			<div class="layui-row">
				<div class="top">
					<div class="position_lead">
						...
						<a class="on"><?php echo session('language') == '中文'? '来稿确认' : 'Feasibility'; ?></a>
					</div>
					<div class="global_btn">
						<a class="layui-btn com_btn" id="scxzpl"><?php echo session('language') == '中文'? '市场行政经理批准' : 'Head of Sales Admin'; ?></a>
	...
	$(function(){
		...
		$(".addCondition").live("click",function(){
			index = parent.layer.open({ //在父窗口打开
				...
				area : ['700px' , '560px'],
				content: "<?php echo url('condition'); ?>",
				end: function () {
					...
					// 表格重载
					table.reload('test', {
						url: "<?php echo url('mk_feseability/index'); ?>",
		...
		if(obj.event === 'del'){
			  layer.confirm('确认删除？', function(index){
				  ...
				  $.ajax({
					  ...
					  url: replaceEditUrlId("<?php echo url('delete', ['id' => 1]); ?>", id),
					  dataType: 'json',
					  // 删除成功
					  success: function (res) {
						  layer.alert(res.msg, {title: '提示'}, function (index) {
		  	...
		  	window.location.href =  replaceEditUrlId("<?php echo url('edit', ['id' => 1]); ?>", id);

	  	} else if(obj.event === 'look'){

		  	window.location.href =  replaceEditUrlId("<?php echo url('read', ['id' => 1]); ?>", id);
	  	}
  	});


	// 市场行政经理 批量批准(多选操作)
	$('#scxzpl').click(function () {
		var checkStatus = table.checkStatus('test');
		var idstr = ''; var cd = checkStatus.data.length;
