<?php /*a:2:{s:70:"D:\largon\laragon\www\ky\application\admin\view\mk_contract\index.html";i:1615860785;s:18:"./layout/list.html";i:1615863909;}*/ ?>
<!doctype html>
<html>
<head>
    <meta charset="utf-8">
			...
			<div class="layui-row">
				<div class="top">
					<div class="position_lead">
						...
						<a class="on"><?php echo session('language') == '中文'? '合同信息' : 'Contract & Administrative Activities'; ?></a>
					</div>
					<div class="global_btn">
						<a href="<?php echo url('mk_contract/create'); ?>" class="layui-btn"><?php echo session('language') == '中文'? '新增' : 'Add'; ?></a>
...
layui.use(['table','form'], function(){
    ...
    tableIns = table.render({
            elem: '#test'
            ,url:"<?php echo url('mk_contract/index'); ?>"
	...
	$('.search_btn').click(function () {
		...
		// 页面 带参跳转 可以记住搜索参数
		window.location.href = "<?php echo url('mk_contract/index'); ?>" + '?field=' + searchfield + '&keyword=' + searchkeyword;
	...
	$(function(){
		...
		$(".addCondition").live("click",function(){
			index = parent.layer.open({ //在父窗口打开
				...
				area : ['700px' , '560px'],
				content: "<?php echo url('mk_contract/condition'); ?>",
				end: function () {
	...
	$('#del').click(function () {
		...
		if(cd !== 0){
			...
			layer.confirm('Confirm？', function(index){
				...
				$.ajax({
					type: 'get',
					url: "<?php echo url('mk_contract/batch_delete'); ?>",
					data: {id : idstr},
					// 删除成功
      ...
      if(obj.event === 'del'){
          layer.confirm('Confirm？', function(index){
              ...
              $.ajax({
                  ...
                  url: replaceEditUrlId("<?php echo url('mk_contract/delete', ['id' => 1]); ?>", id),
                  dataType: 'json',
				  // 删除成功
                  success: function (res) {
                      layer.alert(res.msg, {title: '提示'}, function (index) {
          ...
          window.location.href =  replaceEditUrlId("<?php echo url('mk_contract/edit', ['id' => 1]); ?>", id);

      } else if(obj.event === 'look'){

          window.location.href = replaceEditUrlId("<?php echo url('mk_contract/read', ['id' => 1]); ?>", id);
      }
  });
